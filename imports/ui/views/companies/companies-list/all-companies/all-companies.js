import './all-companies.html';
import '../data-table-actions/company-actions';

import { VZ } from '/imports/startup/both/namespace';
import {Meteor} from 'meteor/meteor'
import {archiveCompanies} from '/imports/api/companies/methods';

Template.allCompanies.onCreated(function () {
...
Template.allCompanies.events({
  'click #select-all'(event, tmpl) {
    ...
    let allChecked = $('tbody input[type="checkbox"]', '#companies-table').prop('checked', this.checked);
  ...
  'click #table_search'(event, tmpl) {
    event.preventDefault();
    $('#companies-table_filter').toggleClass('active');
  ...
  'click #archive-companies'(event, tmpl) {
    event.preventDefault();
    let table = $('#companies-table').DataTable();
    let checkedCompanies = tmpl.checkedCompanies.array();
    Session.set('companiesFormChanged',  false);
    archiveCompanies.call({companyIds: checkedCompanies}, function (error, result) {
      if(!error){
        VZ.notify('Archived');
        tmpl.checkedCompanies.clear();
  ...
  'click #create-company'(event, tmpl){
    event.preventDefault();
    const modalData = {
